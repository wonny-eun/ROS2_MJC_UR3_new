...
void MujocoCameras::update()
{
  ...
  // Recompute derived quantities (camera poses, etc.) on the copy. mjv_copyData duplicates
  // state but does not replace a full mj_forward; rendering without this can yield blank/wrong views.
  mj_forward(mj_model_, mj_camera_data_);
